...
function setup() {
  createCanvas(windowWidth, windowHeight);//window width and window height takes the size the game will run on
  sun= createSprite(width-50,100,10,10)//it is subtracting the value inside from the width
 ...
 text("Score: "+ score,30,50);
  ...
  if(gameState === PLAY){
    ...
    if(keyDown("space") || touches.length>0 && trex.y >= height-100) {
        trex.velocityY = -12;
        jumpSound.play();
      touches=[]//touches is an array data type and it stores all the touched coordinates as a comma separated list
      //arrays can store multiple values in a single variable, the first element of an array should be index 0
    }
